...
def filtro_plano_fourier(xx: np.ndarray, yy: np.ndarray, cfg: dict) -> np.ndarray:
    ...
    if tipo == "circular_rendija":
        ...
        slit  = 1-mascara_circular(xx,yy, cfg["ancho_rendija"])
        return np.minimum(disco, slit)
...
def main():
    ...
    objeto_amplitud = seleccionar_imagen_en_grises(inicio_en_escritorio=True)
    objeto_amplitud = redimensionar_con_padding(objeto_amplitud, nx, ny)
    objeto_fase = seleccionar_imagen_en_grises(inicio_en_escritorio=True)
    objeto_fase = redimensionar_con_padding(objeto_fase,nx,ny)

    objeto=objeto_amplitud*np.exp(1j*objeto_fase)
